#include "Events.h"

namespace IceClean::Gui {

wxDEFINE_EVENT(wxEVT_SCAN_REQUEST, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_SCAN_PROGRESS_UPDATE, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_SCAN_STOP, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_SCAN_COMPLETE, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_MIGRATION_SCAN_PROGRESS, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_MIGRATION_SCAN_REQUEST, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_CLEAN_COMPLETE, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_CLEAN_PROGRESS, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_MIGRATE_COMPLETE, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_MIGRATE_PROGRESS, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_OPERATION_PROGRESS_UPDATE, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_OPERATION_COMPLETE, wxThreadEvent);
wxDEFINE_EVENT(wxEVT_OPERATION_CANCEL, wxThreadEvent);

wxDEFINE_EVENT(wxEVT_NAV_SELECTION_CHANGED, wxCommandEvent);

} // namespace IceClean::Gui
