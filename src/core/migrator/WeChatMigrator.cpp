#include "WeChatMigrator.h"
#include "utils/Win32Util.h"
#include <shlobj.h>

namespace IceClean::Core::Migrator {

std::wstring WeChatMigrator::GetName() const {
    return L"微信缓存迁移";
}

Models::MigrationType WeChatMigrator::GetMigrationType() const {
    return Models::MigrationType::WeChatCache;
}

std::wstring WeChatMigrator::GetWeChatDataPath() const {
    // 微信数据默认在 Documents/WeChat Files/
    std::wstring docsPath = Utils::Win32Util::GetSpecialFolder(CSIDL_PERSONAL);
    if (docsPath.empty()) return L"";

    std::wstring wechatPath = docsPath;
    if (wechatPath.back() != L'\\') wechatPath += L'\\';
    wechatPath += L"WeChat Files";

    if (Utils::FileUtil::Exists(wechatPath)) {
        return wechatPath;
    }

    return L"";
}

bool WeChatMigrator::IsWeChatRunning() const {
    return IsProcessRunning(L"WeChat.exe");
}

std::vector<Models::MigrationItem> WeChatMigrator::Detect() {
    std::vector<Models::MigrationItem> items;

    std::wstring wechatPath = GetWeChatDataPath();
    if (wechatPath.empty()) return items;

    std::wstring systemDrive = Utils::Win32Util::GetSystemDrive();

    // 只检测C盘上的微信数据
    if (wechatPath.size() < 2 || towlower(wechatPath[0]) != towlower(systemDrive[0])) {
        return items;
    }

    // 检查是否已经是联接链接
    if (Utils::JunctionPoint::IsJunction(wechatPath)) {
        return items;
    }

    // 扫描wxid_*子目录
    std::wstring searchPath = wechatPath;
    if (searchPath.back() != L'\\') searchPath += L'\\';
    searchPath += L"wxid_*";

    WIN32_FIND_DATAW findData{};
    HANDLE hFind = FindFirstFileW(searchPath.c_str(), &findData);
    if (hFind == INVALID_HANDLE_VALUE) {
        // 没有wxid子目录，直接迁移整个WeChat Files文件夹
        uint64_t totalSize = Utils::FileUtil::GetFolderSize(wechatPath);

        if (totalSize > 0) {
            Models::MigrationItem item;
            item.name = L"微信数据";
            item.sourcePath = wechatPath;
            item.size = totalSize;
            item.type = Models::MigrationType::WeChatCache;
            item.advice = totalSize > 1024ULL * 1024 * 1024
                ? Models::MigrationAdvice::Recommended
                : Models::MigrationAdvice::Possible;
            item.selected = false;
            item.migrated = false;
            items.push_back(item);
        }

        return items;
    }

    do {
        if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) continue;
        if (wcscmp(findData.cFileName, L".") == 0 || wcscmp(findData.cFileName, L"..") == 0) continue;

        std::wstring wxidPath = wechatPath;
        if (wxidPath.back() != L'\\') wxidPath += L'\\';
        wxidPath += findData.cFileName;

        // 跳过联接链接
        if (Utils::JunctionPoint::IsJunction(wxidPath)) continue;

        uint64_t size = Utils::FileUtil::GetFolderSize(wxidPath);

        Models::MigrationItem item;
        item.name = std::wstring(L"微信 - ") + findData.cFileName;
        item.sourcePath = wxidPath;
        item.size = size;
        item.type = Models::MigrationType::WeChatCache;
        item.advice = size > 500ULL * 1024 * 1024
            ? Models::MigrationAdvice::Recommended
            : Models::MigrationAdvice::Possible;
        item.selected = false;
        item.migrated = false;

        items.push_back(item);
    } while (FindNextFileW(hFind, &findData));

    FindClose(hFind);

    return items;
}

Models::MigrationResult WeChatMigrator::Migrate(const std::vector<Models::MigrationItem>& items,
                                                  const std::wstring& targetDrive,
                                                  std::function<void(const Models::MigrationProgress&)> progressCallback) {
    Models::MigrationResult result;

    if (IsWeChatRunning()) {
        result.errorMessage = L"微信正在运行，请先关闭微信后再迁移";
        return result;
    }

    if (items.empty()) {
        result.errorMessage = L"没有需要迁移的项目";
        return result;
    }

    // 检查目标驱动器空间
    uint64_t driveTotal = 0, driveFree = 0;
    if (Utils::Win32Util::GetDiskSpace(targetDrive, driveTotal, driveFree)) {
        uint64_t totalNeeded = 0;
        for (const auto& item : items) {
            if (item.selected) totalNeeded += item.size;
        }
        if (driveFree < totalNeeded) {
            result.errorMessage = L"目标驱动器空间不足";
            return result;
        }
    }

    int totalItems = static_cast<int>(items.size());
    int migratedCount = 0;
    int failedCount = 0;
    uint64_t totalMigratedSize = 0;

    for (int i = 0; i < totalItems; ++i) {
        if (!items[i].selected) continue;

        if (progressCallback) {
            Models::MigrationProgress progress;
            progress.currentItem = i + 1;
            progress.totalItems = totalItems;
            progress.migratedSize = totalMigratedSize;
            progress.totalSize = items[i].size;
            progress.currentFile = items[i].sourcePath;
            progress.isRunning = true;
            progressCallback(progress);
        }

        bool success = MoveAndCreateJunction(items[i].sourcePath, targetDrive,
                                              progressCallback, i + 1, totalItems);

        if (success) {
            ++migratedCount;
            totalMigratedSize += items[i].size;
        } else {
            ++failedCount;
        }
    }

    result.success = failedCount == 0;
    result.migratedCount = migratedCount;
    result.failedCount = failedCount;
    result.totalMigratedSize = totalMigratedSize;

    if (failedCount > 0 && migratedCount == 0) {
        result.errorMessage = L"所有迁移项目均失败";
    }

    return result;
}

} // namespace IceClean::Core::Migrator
