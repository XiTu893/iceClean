...
public:
    ...
    std::wstring GetDescription() const override { return L"扫描Chrome、Edge、Firefox、Opera、Brave、Vivaldi、360、QQ浏览器等浏览器的缓存文件"; }
...
private:
    ...
    void ScanBrowserCache(const std::wstring& cachePath, Models::ScanCategory& category,
                          const std::atomic<bool>* stopFlag = nullptr, ScanProgressCallback progressCb = nullptr);
    // 检查浏览器是否正在运行
